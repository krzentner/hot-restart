...
def test_module_wrap_no_warnings():
    ...
    child = pexpect.spawn(
        ...
        [
            os.path.join(
                os.path.dirname(__file__), "wrap_warnings", "module_wrap_test.py"
            )
        ],
        encoding="utf-8",
        timeout=10,
    ...
    assert "Could not wrap" not in output, (
        f"Found unwanted warning in output:\n{output}"
    )
    assert "could not get source" not in output, (
        f"Found unwanted warning in output:\n{output}"
    )
...
def test_explicit_wrap_shows_warnings():
    ...
    child = pexpect.spawn(
        ...
        [
            os.path.join(
                os.path.dirname(__file__), "wrap_warnings", "explicit_wrap_test.py"
            )
        ],
        encoding="utf-8",
        timeout=10,
    ...
    # Check that errors/exceptions DO appear for explicit wrap
    # With the new changes, trying to wrap a lambda raises ReloadException
    assert "ReloadException" in output or "Could not" in output, (
        f"Expected error not found in output:\n{output}"
    )

    # The lambda wrap should fail now, but regular function should work
    assert "Regular function: This function has source" in output
